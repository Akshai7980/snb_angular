...
const PAYMENT_SUB_PRODUCT = 'BKSIFT';
const PAYMENT_FUNCTION_CODE = 'CRIFT';
import { downloadAsPdf } from 'src/app/utility/downloadAsPdf';
import { amountUnFormat } from 'src/app/utility/amount-unformat';
@Component({
  selector: 'app-si-own-transfer',
  templateUrl: './si-own-transfer.component.html',
  styleUrls: ['./si-own-transfer.component.scss'],
})
export class SiOwnTransferComponent implements OnInit {
...
// For diable button for amend 
  ...
  ngOnInit(): void {
    if (this.rootScopeData.ownAccountSITransactionObject.amend === "Y") {
  ...
  getToAccounts(account: any | string): void {
    if (account && account.COD_CORECIF) {
      this.paymentService
        .getPaymentOwnBeneAccountApiCall({
          debitCifNo: account.COD_CORECIF,
        })
  ...
  onToAccountSelection(account: any | string): void {
    ...
    let beneAccNo = account && account.OD_ACC_NO ? account.OD_ACC_NO : "";
  ...
  getCurrencies(account: any): void {
    ...
    let debitCcy =
      this.selectedFromAccount[0] && this.selectedFromAccount[0].OD_CCY_CODE
        ...
        : '';
    let beneCcy = account && account.OD_CCY_CODE ? account.OD_CCY_CODE : '';
  ...
  cancel(): void {
    if (this.rootScopeData.ownAccountSITransactionObject) {
  ...
  constructReceipt(referenceNumber: string,data: any): void {  
    if(this.rootScopeData.ownAccountSITransactionObject.amend === "Y"){
     referenceNumber = this.rootScopeData?.ownAccountSITransactionObject?.siBookingRefNo;
    } 
   ...
   let flexiAuth = {
    ...
    this.saveReceiptObject = {
      ...
      "keyValues": [
        ...
        {
          "subHead": "Beneficiary",
          "subValue": this.selectedToAccount[0].ALIAS_NAME ? this.selectedToAccount[0].ALIAS_NAME : "--"
        },
        {
          "subHead": "To Account Number",
          "subValue": this.selectedToAccount[0].OD_ACC_NO ? this.selectedToAccount[0].OD_ACC_NO : "--"
        },
        {
    ...
    let reqObj: any = {
      ...
      debitUnitId:
        this.selectedFromAccount[0] && this.selectedFromAccount[0].UNIT_ID
          ? this.selectedFromAccount[0].UNIT_ID
          : '',
      // paymentAmount: details.amountDetails.transferAmount.replace(/[.,\s]/g,''),
      ...
      inputVersionNum: 1,
      paymentName: 'Own Account Transfer',
      subProduct: 'RECPAY',
      functionCode: this.rootScopeData.ownAccountSITransactionObject.amend === "Y" ? 'AMNDSI':'RECUR',
      paymentType:'BKSIFT',
      debitAvailableBalance:
        this.selectedFromAccount[0] && this.selectedFromAccount[0].CURR_AVAIL_BAL_AMT
          ? this.selectedFromAccount[0].CURR_AVAIL_BAL_AMT
      ...
      beneBank:
        this.selectedToAccount[0] && this.selectedToAccount[0].BANKNAME
          ...
          : '',
      beneBranch:
        this.selectedToAccount[0] && this.selectedToAccount[0].BRANCH_NAME
          ? this.selectedToAccount[0].BRANCH_NAME
          : '',
      beneAccType:
        this.selectedToAccount[0] && this.selectedToAccount[0].OD_ACC_TYPE_2
          ? this.selectedToAccount[0].OD_ACC_TYPE_2
          : '',
      beneCountry:
        this.selectedToAccount[0] && this.selectedToAccount[0].COUNTRY
          ? this.selectedToAccount[0].COUNTRY
          : '',
      beneAccName:
        this.selectedToAccount[0] && this.selectedToAccount[0].OD_ACC_NAME
          ? this.selectedToAccount[0].OD_ACC_NAME
          : '',
      beneCurrencyCode:
        this.selectedToAccount[0] && this.selectedToAccount[0].OD_CCY_CODE
          ? this.selectedToAccount[0].OD_CCY_CODE
          : '',
      beneAccNo:
        this.selectedToAccount[0] && this.selectedToAccount[0].OD_ACC_NO
          ? this.selectedToAccount[0].OD_ACC_NO
          : '',
      authRuleId:
      ...
      txnNo : this.rootScopeData.ownAccountSITransactionObject.amend === "Y" ? this.rootScopeData.ownAccountSITransactionObject.siBookingRefNo : '' ,
      hostRefNo : this.rootScopeData.ownAccountSITransactionObject.amend === "Y" ? this.rootScopeData.ownAccountSITransactionObject.siBookingRefNo :'',
      summaryCifNo :  this.rootScopeData.ownAccountSITransactionObject.cifNo
  ...
  checkSecondFactorAuth() {
    ...
    let reqObj = {
      ...
      beneCurrencyCode: this.selectedToAccount[0].OD_CCY_CODE,
      paymentCurrency: this.selectedToAccount[0].OD_CCY_CODE,
      subProduct: 'RECPAY',
      functionCode: 'RECUR',
      debitUnitId: this.selectedFromAccount[0].UNIT_ID
    };
...
getClonePaymentDebitAccount() {
  ...
  this.paymentService.getPaymentDebitAccountApiCall({ moduleId: reqObj.moduleId }).subscribe((response: any) => {
    if (response) {
      ...
      // debugger;
      if (this.rootScopeData.ownAccountSITransactionObject.debitAccount) {
        for (let i = 0; i < this.debitData.length; i++) {
          // debugger;
          if (this.rootScopeData.ownAccountSITransactionObject.debitAccount === this.debitData[i].OD_ACC_NO) {
  ...
  this.paymentService.getPaymentOwnBeneAccountApiCall(reqObj).subscribe((response: any) => {
    if(response){
      ...
      for (let i = 0; i < this.beneData.length; i++) {
        // debugger;
        if (this.rootScopeData.ownAccountSITransactionObject.benefAcNo === this.beneData[i].OD_ACC_NO) {
    ...
    let beneCcy = this.selectedToAccount && this.selectedToAccount[0] && this.selectedToAccount[0].OD_CCY_CODE ? this.selectedToAccount[0].OD_CCY_CODE : "";
    this.amountTransferCurrencies = [];
    this.amountTransferCurrencies.push(debitCcy)
    this.loadForm = true;
    this.paymentAmtObj.amount =  this.rootScopeData.ownAccountSITransactionObject.siAmt.trim();
    ...
    let amendStartDate = '';
    amendStartDate = this.rootScopeData.ownAccountSITransactionObject.firstPymtDat;
    let amendLastDate = '';
    amendLastDate = this.rootScopeData.ownAccountSITransactionObject.lastPymtDt;
    ...
    this.standingInstructionsObject.numberOfInstances = this.rootScopeData.ownAccountSITransactionObject.numberOfInstances;
...
//Amend Payments End
  ...
  downloadPdf(values:any)
      ...
      [
        ...
        { type: 'text', value:this.selectedFromAccount[0].USER_ID ? this.selectedFromAccount[0].USER_ID : '', y:75},
        ...
        { type: 'text', value:this.selectedToAccount[0].ALIAS_NAME? this.selectedToAccount[0].ALIAS_NAME : '', y:115},
        { type: 'text', value:this.selectedToAccount[0].OD_ACC_NO? this.selectedToAccount[0].OD_ACC_NO : '', y:125},
      ...
      if(SelectedType === 'save'){
        this.pdfData.push(
          { type: 'save', value:'siOwnTransfer.pdf'}
       )
      }       
       else if(SelectedType === 'print'){
        this.pdfData.push(
          { type: 'print', value:'siOwnTransfer.pdf'}
       )
      }
