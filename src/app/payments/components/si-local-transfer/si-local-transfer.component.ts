...
const PAYMENT_SUB_PRODUCT = 'BKSRNT';
const PAYMENT_FUNCTION_CODE = 'CRRNTS';
import { downloadAsPdf } from 'src/app/utility/downloadAsPdf';
import { amountUnFormat } from 'src/app/utility/amount-unformat';

@Component({
  selector: 'app-si-local-transfer',
  templateUrl: './si-local-transfer.component.html',
  styleUrls: ['./si-local-transfer.component.scss'],
})
export class SiLocalTransferComponent implements OnInit {
...
amendEnabled:boolean=false;
  ...
  setInstruction1:string='';
  setInstruction2:string='';
  constructor(
  ...
  getFromAccounts(transferModuleId: string): void {
    ...
    this.paymentService
      ...
      .subscribe(
        (response: any) => {
          this.isLoadingComplete = true;
          response.DATA.ALL_RECORDS = response.DATA.ALL_RECORDS.filter((x:any) => x.OD_CCY_CODE === "SAR");
  ...
  onFromAccountSelection(account: any | string): void {
    ...
    if (typeof account === 'string' && account === 'iconClick') {
      ...
      this.showInstruction = account && account.tag70 ? account.tag70:'';
      this.getDailyLimit();
    ...
    if (account) {
      this.paymentService
        .getPaymentBeneAccountApiCall({
          ...
          functionCode: PAYMENT_FUNCTION_CODE ? PAYMENT_FUNCTION_CODE : '',
        })
  ...
  cancel(): void {
    if (this.rootScopeData.localSITransactionObject) {
      ...
      this.rootScopeData.changeHeading = 'Standing Orders';
      this.getFromAccounts("PMTACCLKPSI");
    }
  }
  ...
  constructReceipt(referenceNumber: string, data : any): void {
    //debugger;
    if(this.rootScopeData.localSITransactionObject.amend === "Y"){ 
      referenceNumber  =  this.rootScopeData?.localSITransactionObject?.siBookingRefNo;
    }
    ...
    this.receiptData = {
      msg1: message1,
      // msg2: 'LBL_LCL_BAK_TRSFR_PNDNG_FR_APPROVAL_MSG',
      msg2: message2,
    ...
    this.saveReceiptObject = {
      ...
      "keyValues": [
        ...
        {
          "subHead": "Beneficiary",
          "subValue": this.selectedToAccount[0].beneaccName ? this.selectedToAccount[0].beneaccName : "--"
        },
        {
  ...
  generateRequestPayloadForSubmit(): any {
    ...
    let reqObj: any = {
      ...
      inputVersionNum: 1,
      paymentName: 'Local Transfer',
      subProduct: 'RECPAY',
      functionCode: this.rootScopeData.localSITransactionObject.amend === "Y" ? 'AMNDSI':'RECUR',
      paymentType: 'BKSRNT',
      debitAvailableBalance:
        this.selectedFromAccount[0] && this.selectedFromAccount[0].CURR_AVAIL_BAL_AMT
          ? this.selectedFromAccount[0].CURR_AVAIL_BAL_AMT
      ...
      beneAccNo:
        this.selectedToAccount[0] && this.selectedToAccount[0].beneAccNo
          ...
          : '',
      beneSwiftCode: this.selectedToAccount[0] && this.selectedToAccount[0].swiftCode ? this.selectedToAccount[0].swiftCode : '',
      authRuleId:
      ...
      txnNo : this.rootScopeData.localSITransactionObject.amend === "Y" ? this.rootScopeData.localSITransactionObject.siBookingRefNo : '' ,
      hostRefNo : this.rootScopeData.localSITransactionObject.amend === "Y" ? this.rootScopeData.localSITransactionObject.siBookingRefNo :'',
      summaryCifNo :  this.rootScopeData.localSITransactionObject.cifNo
  ...
  getClonePaymentDebitAccount() {
    ...
    this.paymentService.getPaymentDebitAccountApiCall({ moduleId: reqObj.moduleId }).subscribe((response: any) => {
      if (response) {
        ...
        // debugger;
        if (this.rootScopeData.localSITransactionObject.debitAccount) {
          for (let i = 0; i < this.debitData.length; i++) {
            // debugger;
            if (this.rootScopeData.localSITransactionObject.debitAccount === this.debitData[i].OD_ACC_NO) {
  ...
  getCloneBeneInfo() {
    ...
    reqObj.functionCode = PAYMENT_FUNCTION_CODE ? PAYMENT_FUNCTION_CODE : ''
    ...
    this.paymentService.getPaymentBeneAccountApiCall(reqObj).subscribe((response: any) => {
      if(response){
        ...
        for (let i = 0; i < this.beneData.length; i++) {
          // debugger;
          if (this.rootScopeData.localSITransactionObject.benefAcNo === this.beneData[i].beneAccNo) {
      ...
      this.amountTransferCurrencies.push(debitCcy)
      this.loadForm = true;
      this.paymentAmtObj.amount =  this.rootScopeData.localSITransactionObject.siAmt.trim();
      ...
      // this.standingInstructionDetails.amountDetails.transferAmount = this.paymentAmtObj.amount;
      // this.standingInstructionDetails.amountDetails.transferCurrency = this.paymentAmtObj.currency;
      ...
      let amendStartDate = '';
      amendStartDate = this.rootScopeData.localSITransactionObject.firstPymtDat;
      let amendLastDate = '';
      amendLastDate = this.rootScopeData.localSITransactionObject.lastPymtDt;
      // let convStartDate = new Date(amendStartDate);
       ...
       this.standingInstructionsObject.numberOfInstances = this.rootScopeData.localSITransactionObject.numberOfInstances
       this.purposeCode = this.rootScopeData.standingOrderDetails.details.purposeDesc;
       //debugger
       this.setInstruction1 = this.rootScopeData.standingOrderDetails.details.instructionMsg1;
       this.setInstruction2 = this.rootScopeData.standingOrderDetails.details.instructionMsg2;
  ...
  downloadPdf(values: any) {
    ...
    this.pdfData =
      [
        ...
        { type: 'text', value: this.selectedToAccount[0].aliasName ? this.selectedToAccount[0].aliasName : '', y: 115 },
