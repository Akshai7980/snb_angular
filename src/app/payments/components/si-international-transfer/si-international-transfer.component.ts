...
const PAYMENT_SUB_PRODUCT = 'TELTRF';
const PAYMENT_FUNCTION_CODE = 'CRTTT';
import { downloadAsPdf } from 'src/app/utility/downloadAsPdf';
import { amountUnFormat } from 'src/app/utility/amount-unformat';

@Component({
  selector: 'app-si-international-transfer',
  templateUrl: './si-international-transfer.component.html',
  styleUrls: ['./si-international-transfer.component.scss'],
})
export class SiInternationalTransferComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.rootScopeData.internationalSITransactionObject.amend === "Y") {
  ...
  cancel(): void {
    if (this.rootScopeData.internationalSITransactionObject) {
  ...
  constructReceipt(referenceNumber: string, data:any): void {
    if(this.rootScopeData.internationalSITransactionObject.amend === "Y"){
      referenceNumber = this.rootScopeData.internationalSITransactionObject.siBookingRefNo;
    }
  ...
  generateRequestPayloadForSubmit(): any {
    ...
    let reqObj: any = {
      ...
      inputVersionNum: 1,
      paymentName: 'International Transfer',
      subProduct: 'RECPAY',
      functionCode: this.rootScopeData.internationalSITransactionObject.amend === "Y" ? 'AMNDSI':'RECUR',
      paymentType:'TELTRF',
      debitAvailableBalance:
        this.selectedFromAccount[0] && this.selectedFromAccount[0].CURR_AVAIL_BAL_AMT
          ? this.selectedFromAccount[0].CURR_AVAIL_BAL_AMT
      ...
      txnNo : this.rootScopeData.internationalSITransactionObject.amend === "Y" ? this.rootScopeData.internationalSITransactionObject.siBookingRefNo : '' ,
      hostRefNo : this.rootScopeData.internationalSITransactionObject.amend === "Y" ? this.rootScopeData.internationalSITransactionObject.siBookingRefNo :'',
      summaryCifNo :this.rootScopeData.internationalSITransactionObject.cifNo
   ...
   getClonePaymentDebitAccount() {
    ...
    this.paymentService.getPaymentDebitAccountApiCall({ moduleId: reqObj.moduleId }).subscribe((response: any) => {
      if (response) {
        ...
        // debugger;
        if (this.rootScopeData.internationalSITransactionObject.debitAccount) {
          for (let i = 0; i < this.debitData.length; i++) {
            // debugger;
            if (this.rootScopeData.internationalSITransactionObject.debitAccount === this.debitData[i].OD_ACC_NO) {
    ...
    this.paymentService.getPaymentBeneAccountApiCall(reqObj).subscribe((response: any) => {
      if(response){
        ...
        for (let i = 0; i < this.beneData.length; i++) {
          // debugger;
          if (this.rootScopeData.internationalSITransactionObject.benefAcNo === this.beneData[i].beneAccNo) {
      ...
      this.amountTransferCurrencies.push(debitCcy)
      this.loadForm = true;
      this.paymentAmtObj.amount =  this.rootScopeData.internationalSITransactionObject.siAmt.trim();
      ...
      let amendStartDate = '';
      amendStartDate = this.rootScopeData.internationalSITransactionObject.firstPymtDat;
      let amendLastDate = '';
      amendLastDate = this.rootScopeData.internationalSITransactionObject.lastPymtDt; 
      // let convStartDate = new Date(amendStartDate);
       ...
       this.standingInstructionsObject.numberOfInstances = this.rootScopeData.internationalSITransactionObject.numberOfInstances
