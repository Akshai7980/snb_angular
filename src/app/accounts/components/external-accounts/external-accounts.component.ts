...
export class ExternalAccountsComponent implements OnInit {
  ...
  constructor(private accountService:AccountDetailsService, private router:Router) { 
    this.rootScopeData.activeTabName='externalaccount' }
    ...
    ngOnInit(): void {
      ...
      this.getCasaInfo();
        ...
        getCasaInfo(){
          this.accountService.getexternalActDtls().subscribe((res:any)=>{
            this.dataSource = res.DATA.ALL_RECORDS;
            this.rootScopeData.externalaccSummaryCount = res.DATA.TOTAL_COUNT;
            // console.log(res)
