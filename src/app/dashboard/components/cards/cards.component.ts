...
export class CardsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['cardNumber','cardHolderName' ,'cardCategory', 'creditLimit', 'billedAmount','action'];
  ...
  ngOnInit(): void {
    ...
    this.getCardsSummary();
    this.isViewBalClicked[this.dataSourceLength] = false;
    this.printSection="dashboardCardsContainer";
  }

  getTableHeight() {
    let that = this;
    window.setTimeout(function() {
      that.contentHeight = document.getElementById("cardsexpandCollapseElement")?.clientHeight + '';
    }, 1000);
  }

  triggerSearchFilter(event:any): void {
    showFilteredRows('cardsDefaultCntr', event.target.value); 
  }

  getCardsSummary(){
    this.dashboardService.cardsSummaryApiCall().subscribe(
      (data: any) =>{
		  if(data.HEADER_VALUE != undefined){
		  	this.responseHeader = data.HEADER_VALUE;
		  }
      this.cardSummaryData = data;
      this.dataSource = this.cardSummaryData.DATA.ALL_RECORDS;
      this.dataSourceLength=this.cardSummaryData.DATA.ALL_RECORDS.length;
  ...
  onClickExpand(event:any){
    this.isCardsExpand = !this.isCardsExpand;
    // var setContainer = 'cardsexpandCollapseElement';
    // var setToolsElement ='cardssummaryToolsElement';
    // expandCollapse(this.isCardsExpand,event,setContainer,setToolsElement)
  }
