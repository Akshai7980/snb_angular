...
@Component({
  selector: 'app-sadad-moi',
  templateUrl: './sadad-moi.component.html',
  styleUrls: ['./sadad-moi.component.scss']
})
export class SadadMoiComponent implements OnInit {
    ...
    switch (comp) {
      case 'debitTo':
        if (val ==='iconClick') {
          ...
          if(Number(amountUnFormat(val.CURR_AVAIL_BAL_AMT))<=0){
  ...
  getDebitData() {
    this.sadadService.getSadadMOIDebitApiCall().subscribe((debData: any) => {
      if (debData) {
  ...
  proceedNext() {
    ...
    this.initReqParam.subPrdCode='SADMOIPAY';
  ...
  onSubmit() {
    ...
    this.debitAmtReceipt = this.payToObject[0].receiptAmt
      ...
      reqObj.subProductCode="SADMOIPAY";
      reqObj.functionCode="SAMITXN";
      ...
      if (this.payToObject) {
        for (let i = 0; i < this.payToObject.length; i++) {
          let selectedValues = {
            ...
            "BILLER_ID": String(this.payToObject[i].billerId),
            "DYNAMIC_DATA": this.payToObject[i].dynamicObject,
          }
          selectedDataValues.push(selectedValues)
        }
      }
    this.sadadService.getSadadMoiSubmit(
      selectedDataValues,
      this.selectedDebitObj,
      reqObj,
      this.secAuthRef,
      this.dynamicParamsForPayload
      )
  ...
  constructReceiptData(refNumber: any, dataValue : any) {
    ...
    this.receiptData = {
      "msg1": message1,
      // "msg2": this.moreTransaction ? "LBL_LCL_BAK_TRSFR_PNDNG_FR_APPROVAL_MSG" : "LBL_SADAD_MOI_PMT_PENDNG_FR_APPROVAL_MSG",
      // "msg2": this.moreTransaction ? "LBL_LOCAL_TRSFR__IS_INT_SUCCESS" : "LBL_SADAD_MOI_PMT_IS_INT_SUCCESS",
      "msg2": message2,
      ...
      "receiptDetails": [
        ...
        },
        {
          ...
          "fieldDetails": [
            {
              "dispKey": "LBL_BILLER",
              "dataKey":this.moreTransaction ? this.transferData[0].biller : this.payToObject[0].biller
            },
            {
              "dispKey": "LBL_SERVICE_TYPE",
              "dataKey": this.moreTransaction ? this.transferData[0].serviceType : this.payToObject[0].serviceType
            },
            {
              "dispKey": "LBL_AMOUNT",
              "dataKey": this.moreTransaction ? this.transferData[0].receiptAmt : this.payToObject[0].receiptAmt
            },
            {
    ...
    this.saveReceiptObject = {
      ...
      // "Description": this.moreTransaction ? (this.translateService.instant("LBL_LOCAL_TRSFR__IS_INT_SUCCESS")) : (this.translateService.instant("LBL_SADAD_MOI_PMT_IS_INT_SUCCESS")),
      ...
      "keyValues": [
        ...
        {
          "subHead": "Biller",
          "subValue": this.moreTransaction ? this.transferData[0].biller : this.payToObject[0].biller
        },
        {
          "subHead": "Service Type",
          "subValue": this.moreTransaction ? this.transferData[0].serviceType : this.payToObject[0].serviceType
        },
        {
          "subHead": 'Amount',
          "subValue": this.moreTransaction ? this.transferData[0].receiptAmt : this.payToObject[0].receiptAmt
        }
      ],
      "pagecall":"sadadmoi",
      "addDynamicHead": this.dynamicParamsForPayload[0].englishName,
      "addDynamicValue": this.dynamicParamsForPayload[0].value,
      "refNo":refNumber
    }

    //console.log(this.payToObject)
    this.hideAll = true;
  ...
  [
    ...
    { type: 'text', value:this.selectedDebitObj.OD_ACC_NO ? this.selectedDebitObj.USER_ID : '', y:75},
    ...
    { type: 'text', value: this.payToObject[0].receiptAmt ?  this.payToObject[0].receiptAmt : '', y:135},
    { type: 'text', value: this.debitAmtReceipt ?  this.debitAmtReceipt : '', y:145},
    { type:'setFont', fontName:'Amiri-Regular', fontStyle:'normal'},
